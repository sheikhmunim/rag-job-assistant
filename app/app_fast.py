...
# ---------------------------------------------------------------------
# Make sure we can import from src/ (for USER_PROFILE)
# ---------------------------------------------------------------------
ROOT = Path(__file__).resolve().parents[1]
SRC_DIR = ROOT / "src"
sys.path.append(str(SRC_DIR))

# from config import OUT_DIR  # no longer needed when using API
try:
...
# ---------------------------------------------------------------------
# Helper: call FastAPI /jd endpoint
# ---------------------------------------------------------------------
def call_jd_api(jd_text: str) -> dict:
    """Call the FastAPI /jd endpoint instead of local generate_all_from_jd()."""
    url = f"{API_BASE}/jd"
    resp = requests.post(url, json={"jd_text": jd_text})
...
# ---------------------------------------------------------------------
# Run pipeline on click (via FastAPI)
# ---------------------------------------------------------------------
if generate_btn:
    if not jd_text.strip():
        st.error("Please paste a job description first.")
    else:
        with st.spinner("Calling FastAPI backend and generating content..."):
            try:
                result = call_jd_api(jd_text)
            except Exception as e:
                st.error(f"Pipeline / API failed: {e}")
                traceback.print_exc()
                st.stop()

        st.session_state.result = result
        st.session_state.skills_text = result.get("skills", "")
        st.session_state.cover_text = result.get("cover", "")
        st.session_state.emails_text = result.get("emails", "")
        st.session_state.ats_text = result.get("ats", "")
